...
function Show() {
  ...
  data?.shows[0]?.seatsBooked?.forEach(el=>document.getElementsByName(el)[0].style.color='pink')
  ...
  return data.shows.length==0 ? (<div  style={{width:'100%',height:'calc(100vh - 100px)',display: 'flex',justifyContent:'centre',alignItems:'center'}}>
                                              <div onClick={()=>{window.history.go(-1)}}  style={{textDecoration:'none',color:'black',padding:'20px',margin: 'auto',background: 'aliceblue'}} to='/'>No Shows Found Go Back</div>
                                 </div> )
                              :(
    <>
